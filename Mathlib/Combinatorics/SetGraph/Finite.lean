...
necessary. For example, `SetGraph.neighborFinset v` only requires that `v` have
finitely many neighbors.

## Main definitions

* `SetGraph.edgeFinset` is the `Finset` of edges in a graph, if `edgeSet` is finite
* `SetGraph.neighborFinset` is the `Finset` of vertices adjacent to a given vertex,
   if `neighborSet` is finite
* `SetGraph.incidenceFinset` is the `Finset` of edges containing a given vertex,
...
variable {V : Type*} (G : SetGraph V) {e : Sym2 V} {G₁ G₂ : SetGraph V}
section Verts
...
variable (G : SetGraph V) [DecidableEq V] [Fintype G.verts] [DecidableRel G.Adj]

abbrev Subgraph G := {H : SetGraph V // H ≤ G}
...
variable {G₁ G₂ : SetGraph V} [Fintype G.edgeSet] [Fintype G₁.edgeSet] [Fintype G₂.edgeSet]
...
@[simp]
theorem edgeFinset_bot : (⊥ : SetGraph V).edgeFinset = ∅ := by simp [edgeFinset]
...
def finiteAtOfSubgraph {G G' : SetGraph V} [DecidableRel G.Adj] (h : G ≤ G') (v : G.verts)
    [Fintype (G'.neighborSet v)] : Fintype (G.neighborSet v) :=
  Set.fintypeSubset (G'.neighborSet v) (neighborSet_subset_of_subgraph h v)

instance (G : SetGraph V) [Fintype G.verts] (v : V) [DecidablePred (· ∈ G.neighborSet v)] :
    Fintype (G.neighborSet v) :=
  Set.fintypeSubset G.verts (neighborSet_subset_verts G v)


theorem degree_le' (G G' : SetGraph V) (h : G ≤ G') (v : V) [Fintype (G.neighborSet v)]
...
theorem degree_eq_one_iff_unique_adj {G : SetGraph V} {v : V} [Fintype (G.neighborSet v)] :
...
-- theorem IsRegularOfDegree.compl [Fintype V] [DecidableEq V] {G : SetGraph V} [DecidableRel G.Adj]
...
lemma minDegree_le_minDegree {H : SetGraph V} [DecidableRel G.Adj] [DecidableRel H.Adj]
...
theorem Adj.card_commonNeighbors_lt_degree {G : SetGraph V} [DecidableRel G.Adj] {v w : V}
...
theorem card_commonNeighbors_top [DecidableEq V] {v w : V} (h : v ≠ w) :
    Fintype.card ((⊤ : SetGraph V).commonNeighbors v w) = Fintype.card V - 2 := by
...
variable {s : Set V} [DecidablePred (· ∈ s)] [Fintype V] {G : SetGraph V} [DecidableRel G.Adj]
