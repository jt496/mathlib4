...
-/
import Mathlib.Combinatorics.SetGraph.Clique
...
variable {V : Type u} (G : SetGraph V) {n : ℕ}
...
-/
abbrev Coloring (α : Type v) := G →g (⊤ : SetGraph α)
...
/-- Construct a term of `SetGraph.Coloring` using a function that
assigns vertices to colors and a proof that it is as proper coloring.

(Note: this is a definitionally the constructor for `SetGraph.Hom`,
...
noncomputable instance [Fintype V] [Fintype α] : Fintype (Coloring G α) := by
  classical
  change Fintype (RelHom G.Adj (⊤ : SetGraph α).Adj)
...
lemma chromaticNumber_eq_biInf {G : SetGraph V} :
    G.chromaticNumber = ⨅ n ∈ setOf G.Colorable, (n : ℕ∞) := rfl

lemma chromaticNumber_eq_iInf {G : SetGraph V} :
    G.chromaticNumber = ⨅ n : {m | G.Colorable m}, (n : ℕ∞) := by
  rw [chromaticNumber, iInf_subtype]

lemma Colorable.chromaticNumber_eq_sInf {G : SetGraph V} {n} (h : G.Colorable n) :
...
theorem colorable_of_fintype (G : SetGraph V) [Fintype V] : G.Colorable (Fintype.card V) :=
...
theorem Colorable.of_embedding {V' : Type*} {G' : SetGraph V'} (f : G ↪g G') {n : ℕ}
...
theorem colorable_chromaticNumber_of_fintype (G : SetGraph V) [Finite V] :
    G.Colorable (ENat.toNat G.chromaticNumber) := by
  cases nonempty_fintype V
  exact colorable_chromaticNumber G.colorable_of_fintype

theorem chromaticNumber_le_one_of_subsingleton (G : SetGraph V) [Subsingleton V] :
...
theorem chromaticNumber_eq_zero_of_isempty (G : SetGraph V) [IsEmpty V] :
    G.chromaticNumber = 0 := by
  rw [← nonpos_iff_eq_zero, ← Nat.cast_zero, chromaticNumber_le_iff_colorable]
  apply colorable_of_isEmpty

theorem isEmpty_of_chromaticNumber_eq_zero (G : SetGraph V) [Finite V]
...
theorem Colorable.mono_left {G' : SetGraph V} (h : G ≤ G') {n : ℕ} (hc : G'.Colorable n) :
    G.Colorable n :=
  ⟨hc.some.comp (.ofLE h)⟩

theorem chromaticNumber_le_of_forall_imp {V' : Type*} {G' : SetGraph V'}
...
theorem chromaticNumber_mono (G' : SetGraph V)
    (h : G ≤ G') : G.chromaticNumber ≤ G'.chromaticNumber :=
  chromaticNumber_le_of_forall_imp fun _ => Colorable.mono_left h

theorem chromaticNumber_mono_of_embedding {V' : Type*} {G' : SetGraph V'}
...
theorem chromaticNumber_bot [Nonempty V] : (⊥ : SetGraph V).chromaticNumber = 1 := by
  have : (⊥ : SetGraph V).Colorable 1 := ⟨.mk 0 <| by simp⟩
  exact this.chromaticNumber_le.antisymm <| Order.one_le_iff_pos.2 <| chromaticNumber_pos this

@[simp]
theorem chromaticNumber_top [Fintype V] : (⊤ : SetGraph V).chromaticNumber = Fintype.card V := by
...
theorem IsClique.card_le_of_coloring {s : Finset V} (h : G.IsClique s) [Fintype α]
  ...
  have : G.IsNClique s.card s := ⟨h, rfl⟩
  let f := topEmbeddingOfNotCliqueFree this.not_cliqueFree
  exact card_fin s.card ▸ (Fintype.card_le_of_injective _ (C.comp f.toHom).injective_of_top_hom)
