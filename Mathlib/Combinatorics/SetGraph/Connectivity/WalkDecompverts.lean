...
import Mathlib.Combinatorics.SetGraph.Walkverts
...
namespace SetGraph.Walk

universe u

variable {V : Type u} {G : SetGraph V} {v w u : V}
...
def takeUntil {v w : V} : ∀ (p : G.Walk v w) (u : V), u ∈ p.support → G.Walk v u
  | nil hv, u, h => by rw [(mem_support_nil_iff hv).mp h]; exact Walk.nil hv
  | cons r p, u, h =>
    if hx : v = u then
      hx ▸ Walk.nil r.mem_verts
    else
...
@[simp] theorem takeUntil_nil {u : V} (hu : u ∈ G.verts) {h} :
    takeUntil (nil hu : G.Walk u u) u h = nil hu := rfl
...
lemma takeUntil_first (p : G.Walk u v) :
    p.takeUntil u p.start_mem_support = .nil p.start_mem_verts :=
    by cases p <;> simp [Walk.takeUntil]
...
The lemma `SetGraph.Walk.count_support_takeUntil_eq_one` specifies where this split occurs. -/
@[simp]
...
theorem mem_support_iff_exists_append {V : Type u} {G : SetGraph V} {u v w : V}
...
theorem mem_support_iff_exists_getVert {u v w : V} {p : G.Walk v w} :
  ...
  constructor
  · intro h
    obtain ⟨q, r, hqr⟩ := SetGraph.Walk.mem_support_iff_exists_append.mp h
    use q.length
