...
variable {V : Type*} (G : SetGraph V) (s t : V)

namespace Iso

variable {G} {W : Type*} {G' : SetGraph W} (f : G ≃g G')
...
def replaceVertex (h : t ∈ G.verts): SetGraph V where
  verts := G.verts
...
def edge : SetGraph V := fromEdgeSet {s(s, t)}
...
theorem edgeSet_sup_edge_le {H : SetGraph V} (hH : H ≤ G ⊔ edge s t)
