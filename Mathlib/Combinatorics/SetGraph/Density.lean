...
import Mathlib.Combinatorics.SetGraph.Basic
...
* `SetGraph.interedges`: Finset of edges of a graph.
* `SetGraph.edgeDensity`: Edge density of a graph.
-/
...
variable (G : SetGraph α) [DecidableRel G.Adj] {s s₁ s₂ t t₁ t₂ : Finset α} {a b : α}
...
theorem card_interedges_add_card_interedges_compl (h : Disjoint s t)
  (hs : s.toSet ⊆ G.verts) (ht : t.toSet ⊆ G.verts) :
  ...
  have : {e ∈ s ×ˢ t | Gᶜ.Adj e.1 e.2} = {e ∈ s ×ˢ t | ¬G.Adj e.1 e.2} := by
    ...
    have h1 : x.1 ∈ G.verts := by apply hs ; simpa using hx.1
    have h2 : x.2 ∈ G.verts := by apply ht ; simpa using hx.2
    constructor <;> intro h'
    · exact h'.1
    · exact ⟨h', h1, h2⟩
...
theorem edgeDensity_add_edgeDensity_compl (hs : s.Nonempty) (ht : t.Nonempty) (h : Disjoint s t)
  ...
  · exact mod_cast card_interedges_add_card_interedges_compl _ h hs ht
  · positivity
...
/-- Extension for the `positivity` tactic: `Rel.edgeDensity` and `SetGraph.edgeDensity` are
always nonnegative. -/
@[positivity]
unsafe def positivity_edge_density : expr → tactic strictness
  ...
  | q(SetGraph.edgeDensity $(G) $(s) $(t)) =>
    nonnegative <$> mk_mapp `` SetGraph.edgeDensity_nonneg [none, G, none, s, t]
  | e =>
    pp e >>=
      fail ∘
        format.bracket "The expression `"
          "` isn't of the form `Rel.edgeDensity r s t` nor `SetGraph.edgeDensity G s t`"
