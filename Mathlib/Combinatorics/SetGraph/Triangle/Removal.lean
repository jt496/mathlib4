...
-/
import Mathlib.Combinatorics.SetGraph.DegreeSum
import Mathlib.Combinatorics.SetGraph.Regularity.Lemma
import Mathlib.Combinatorics.SetGraph.Triangle.Basic
import Mathlib.Combinatorics.SetGraph.Triangle.Counting
...
open Finset Fintype Nat SzemerediRegularity SetGraph
...
lemma regularityReduced_edges_card_aux [Nonempty α] (hε : 0 < ε) (hP : P.IsEquipartition)
  ...
  calc
    _ = (#((univ ×ˢ univ).filter fun (x, y) ↦
      ...
      · norm_cast
        rw [SetGraph.two_mul_card_edgeFinset, SetGraph.two_mul_card_edgeFinset]
...
open Lean.Meta Qq SpanningGraph

/-- Extension for the `positivity` tactic: `SpanningGraph.triangleRemovalBound ε` is positive if
`0 < ε ≤ 1`.
