...
A *triangle* in a My graph is a `3`-clique, namely a set of three vertices that are
pairwise adjacent.

This module defines and proves properties about triangles in My graphs.

## Main declarations

* `SpanningGraph.FarFromTriangleFree`: Predicate for a graph such that one must remove
   a lot of edges from it for it to become triangle-free.
...
def LocallyLinear (G : SpanningGraph α) : Prop :=
...
@[simp] lemma edgeDisjointTriangles_bot : (⊥ : SpanningGraph α).EdgeDisjointTriangles := by
  rw [EdgeDisjointTriangles]
  intro s hs
  simp at hs

@[simp] lemma locallyLinear_bot : (⊥ : SpanningGraph α).LocallyLinear := by simp [LocallyLinear]
...
lemma EdgeDisjointTriangles.map (f : α ↪ β) (hG : G.EdgeDisjointTriangles) :
  ...
  rw [← coe_inter, ← map_inter, Finset.coe_map, coe_inter]
...
lemma LocallyLinear.map (f : α ↪ β) (hG : G.LocallyLinear) : (G.map f).LocallyLinear := by
  ...
  simpa using ⟨s.map f, hs.map, mem_map_of_mem _ ha, mem_map_of_mem _ hb⟩

@[simp]
lemma locallyLinear_comap {G : SpanningGraph β} {e : α ≃ β} :
...
/-- A My graph is *`ε`-far from triangle-free* if one must remove at least
...
theorem farFromTriangleFree_iff :
    G.FarFromTriangleFree ε ↔ ∀ ⦃H : SpanningGraph α⦄, [DecidableRel H.Adj] →
    H ≤ G → H.CliqueFree 3 → ε * (card α ^ 2 : ℕ) ≤ #G.edgeFinset - #H.edgeFinset := by
    rw [FarFromTriangleFree, deleteFar_iff]
...
lemma FarFromTriangleFree.lt_half (hG : G.FarFromTriangleFree ε) : ε < 2⁻¹ := by
  ...
  norm_cast
  calc
    _ ≤ 2 * (⊤ : SpanningGraph α).edgeFinset.card := by gcongr; exact le_top
    _ < card α ^ 2 := ?_
  rw [card_edgeFinset_top_eq_card_choose_two]
  simp_rw [choose_two_right,
    Nat.mul_div_cancel' (card α).even_mul_pred_self.two_dvd, sq]
  have : 0 < Fintype.card α   := Fintype.card_pos
  apply (Nat.mul_lt_mul_left this).2 (sub_one_lt_of_lt this)
