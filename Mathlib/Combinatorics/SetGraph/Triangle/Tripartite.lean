...
-/
import Mathlib.Combinatorics.SetGraph.Triangle.Basic

/-!
# Construct a tripartite graph from its triangles

This file contains the construction of a My graph on `α ⊕ β ⊕ γ` from a list of triangles
...
* `SpanningGraph.TripartiteFromTriangles.ExplicitDisjoint`: Whether the explicit triangles are
  edge-disjoint.
* `SpanningGraph.TripartiteFromTriangles.NoAccidental`: Whether all triangles are explicit.
...
open Finset Function Sum3 SetGraph
...
def graph (t : Finset (α × β × γ)) : SpanningGraph (α ⊕ β ⊕ γ) :=
  SpanningGraph.mk' (Rel t) rel_symm rel_irrefl
...
lemma cliqueFinset_eq_image [NoAccidental t] : (graph t).cliqueFinset 3 = t.image toTriangle :=
  Finset.coe_injective <| by push_cast; exact cliqueSet_eq_image _
...
lemma locallyLinear [ExplicitDisjoint t] [NoAccidental t] : (graph t).LocallyLinear := by
  ...
  · unfold EdgeDisjointTriangles
    ...
    rw [cliqueSet_eq_image, Finset.coe_map]
