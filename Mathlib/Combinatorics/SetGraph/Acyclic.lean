...
* `SetGraph.IsAcyclic` is a predicate for a graph having no cyclic walks.
* `SetGraph.IsTree` is a predicate for a graph being a tree (a connected acyclic graph).

## Main statements

* `SetGraph.isAcyclic_iff_path_unique` characterizes acyclicity in terms of uniqueness of
  paths between pairs of vertices.
* `SetGraph.isAcyclic_iff_forall_edge_isBridge` characterizes acyclicity in terms of every
  edge being a bridge edge.
* `SetGraph.isTree_iff_existsUnique_path` characterizes trees in terms of existence and
  uniqueness of paths between pairs of vertices from a nonempty vertex type.

## References

The structure of the proofs for `SetGraph.IsAcyclic` and `SetGraph.IsTree`, including
supporting lemmas about `SetGraph.IsBridge`, generally follows the high-level description
...
@[simp] lemma isAcyclic_bot : IsAcyclic (⊥ : SetGraph V) := fun _a _w hw ↦ hw.ne_bot rfl
...
theorem IsAcyclic.path_unique {G : SetGraph V} (h : G.IsAcyclic) {v w : V} (p q : G.Path v w) :
    p = q := by
  obtain ⟨p, hp⟩ := p
  obtain ⟨q, hq⟩ := q
  rw [Subtype.mk.injEq]
...
lemma Preconnected.exists_isTree_le [Finite V] (h : G.Preconnected) : ∃ T ≤ G, IsTree T := by
  obtain ⟨T, hTG, hmin⟩ := {H : SetGraph V | H.Preconnected}.toFinite.exists_minimal_le h
...
lemma Preconnected.exists_isTree_le' [Finite G.verts] (h : G.Preconnected) : ∃ T ≤ G, IsTree T := by
  haveI hf : Finite {H : SetGraph V | H ≤ G} := by sorry
  haveI : Finite {H : SetGraph V | H ≤ G ∧ H.Preconnected} := Set.Finite.subset hf (fun _ h' ↦ h'.1)
  have : G ≤ G ∧ G.Preconnected := ⟨le_refl _, h⟩
  obtain ⟨T, hTG, hmin⟩ := {H : SetGraph V | H ≤ G ∧ H.Preconnected}.toFinite.exists_minimal_le this
...
variable {G}
lemma isTree_coe (hG : G.IsTree) : (G : SetGraph V).IsTree :=
