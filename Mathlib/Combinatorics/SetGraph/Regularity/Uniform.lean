...
import Mathlib.Combinatorics.SetGraph.Density
...
* `SetGraph.IsUniform`: Graph uniformity of a pair of finsets of vertices.
* `SetGraph.nonuniformWitness`: `G.nonuniformWitness ε s t` and `G.nonuniformWitness ε t s`
...
variable (G : SetGraph α) [DecidableRel G.Adj] (ε : 𝕜) {s t : Finset α} {a b : α}
...
variable [DecidableEq α] {A : Finset α} (P : Finpartition A) (G : SetGraph α)
...
theorem nonUniforms_mono {ε ε' : 𝕜} (h : ε ≤ ε') : P.nonUniforms G ε' ⊆ P.nonUniforms G ε :=
  monotone_filter_right _ fun _ => mt <| SetGraph.IsUniform.mono h
...
theorem nonUniforms_bot (hε : 0 < ε) : (⊥ : Finpartition A).nonUniforms G ε = ∅ := by
  ...
  rwa [SetGraph.isUniform_singleton]
...
lemma isUniform_one : P.IsUniform G (1 : 𝕜) := by
  ...
  refine (card_filter_le _
    (fun uv => ¬SetGraph.IsUniform G 1 (Prod.fst uv) (Prod.snd uv))).trans ?_
...
variable (G : SpanningGraph α) [DecidableRel G.Adj]
...
@[simps!] def regularityReduced (ε δ : 𝕜) : SpanningGraph α :=
  SpanningGraph.mk' (fun a b ↦  G.Adj a b ∧ ∃ U ∈ P.parts, ∃ V ∈ P.parts, a ∈ U ∧ b ∈ V ∧ U ≠ V ∧
                       G.IsUniform ε U V ∧ δ ≤ G.edgeDensity U V)
  (fun a b ↦ by
    rintro ⟨ab, U, UP, V, VP, xU, yV, UV, GUV, εUV⟩
    refine ⟨G.symm ab, V, VP, U, UP, yV, xU, UV.symm, GUV.symm, ?_⟩
    rwa [edgeDensity_comm])
  (fun a h ↦ G.loopless a h.1)

instance regularityReduced.instDecidableRel_adj : DecidableRel (G.regularityReduced P ε δ).Adj := by
  unfold regularityReduced;  intro a b; simp; infer_instance

variable {G P}

lemma regularityReduced_le : G.regularityReduced P ε δ ≤ G := le_iff.2 <| fun _ _ ↦ And.left

lemma regularityReduced_mono {ε₁ ε₂ : 𝕜} (hε : ε₁ ≤ ε₂) :
    G.regularityReduced P ε₁ δ ≤ G.regularityReduced P ε₂ δ := le_iff.2 <|
