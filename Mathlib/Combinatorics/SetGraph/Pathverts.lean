...
Authors: Kyle Miller
-/
import Mathlib.Combinatorics.SetGraph.Connectivity.WalkDecompverts
import Mathlib.Combinatorics.SetGraph.Finite
import Mathlib.Combinatorics.SetGraph.Spanning
/-!
...
* `SetGraph.Walk.IsTrail`, `SetGraph.Walk.IsPath`, and `SetGraph.Walk.IsCycle`.

* `SetGraph.Path`

* `SetGraph.Path.map` for the induced map on paths,
  given an (injective) graph homomorphism.

* `SetGraph.Reachable` for the relation of whether there exists
  a walk between a given pair of vertices

* `SetGraph.Preconnected` and `SetGraph.Connected` are predicates
...
* `SetGraph.ConnectedComponent` is the type of connected components of
  a given graph.

* `SetGraph.IsBridge` for whether an edge is a bridge edge

## Main statements

* `SetGraph.isBridge_iff_mem_and_forall_cycle_not_mem` characterizes bridge edges in terms of
...
Use `SetGraph.Walk.IsPath.mk'` for a simpler constructor. -/
...
lemma IsPath.of_adj {G : SetGraph V} {u v : V} (h : G.Adj u v) : h.toWalk.IsPath := by
  aesop
...
lemma IsCycle.three_le_length {v : V} {p : G.Walk v v} (hp : p.IsCycle) : 3 ≤ p.length := by
  ...
  | .cons _ (.cons _ (.cons _ _)) => simp_rw [SetGraph.Walk.length_cons]; omega
...
lemma IsCycle.getVert_injOn {p : G.Walk u u} (hpc : p.IsCycle) :
  ...
  intro n hn m hm hnm
  rw [← SetGraph.Walk.length_tail_add_one
  ...
  have := ((Walk.cons_isCycle_iff _ _).mp hpc).1.getVert_injOn
      ...
      (by simp_all [SetGraph.Walk.getVert_tail, show n - 1 + 1 = n by omega,
...
/-- The reverse of a path is another path.  See also `SetGraph.Walk.reverse`. -/
@[symm, simps]
...
The result is a path, as shown in `SetGraph.Walk.bypass_isPath`.
This is packaged up in `SetGraph.Walk.toPath`. -/
...
/-- Given a walk, produces a path with the same endpoints using `SetGraph.Walk.bypass`. -/
...
@[simp]
theorem mapLe_isTrail {G G' : SetGraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
...
@[simp]
theorem mapLe_isPath {G G' : SetGraph V} (h : G ≤ G') {u v : V} {p : G.Walk u v} :
...
@[simp]
theorem mapLe_isCycle {G G' : SetGraph V} (h : G ≤ G') {u : V} {p : G.Walk u u} :
...
See `SetGraph.reachable_iff_reflTransGen`. -/
...
protected theorem Walk.reachable {G : SetGraph V} {u v : V} (p : G.Walk u v) : G.Reachable u v :=
  ⟨p⟩
...
protected theorem Reachable.map {u v : V} {G : SetGraph V} {G' : SetGraph V'} (f : G →g G')
    (h : G.Reachable u v) : G'.Reachable (f u) (f v) :=
  h.elim fun p => ⟨p.map f⟩

@[mono]
protected lemma Reachable.mono {u v : V} {G G' : SetGraph V}
...
theorem Iso.reachable_iff {G : SetGraph V} {G' : SetGraph V'} {φ : G ≃g G'} {u v : V} :
    G'.Reachable (φ u) (φ v) ↔ G.Reachable u v :=
  ⟨fun r => φ.left_inv u ▸ φ.left_inv v ▸ r.map φ.symm.toHom, Reachable.map φ.toHom⟩

theorem Iso.symm_apply_reachable {G : SetGraph V} {G' : SetGraph V'} {φ : G ≃g G'} {u : V}
    {v : V'} : G.Reachable (φ.symm v) u ↔ G'.Reachable v (φ u) := by
  rw [← Iso.reachable_iff, Iso.apply_symm_apply]

lemma Reachable.mem_subgraphVerts {u v} {H : SetGraph V} (hr : G.Reachable u v)
...
-- @[simp]
-- lemma reachable_bot {u v : V} : (⊥ : SetGraph V).Reachable u v ↔ u = v :=
--   ⟨fun h ↦ h.elim fun p ↦ match p with | .nil => rfl, fun h ↦ h ▸ .rfl⟩

/-- The equivalence relation on G.verts given by `SetGraph.Reachable`. -/
...
theorem Preconnected.map {G : SetGraph V} {H : SetGraph V'} (f : G →g H)
...
@[mono]
protected lemma Preconnected.mono  {G G' : SetGraph V} (h : G ≤ G') (h' : G'.verts ⊆ G.verts)
...
-- lemma bot_preconnected_iff_subsingleton : (⊥ : SetGraph V).Preconnected ↔ Subsingleton V := by
...
-- lemma bot_preconnected [Subsingleton V] : (⊥ : SetGraph V).Preconnected :=
--   bot_preconnected_iff_subsingleton.mpr ‹_›

-- lemma bot_not_preconnected [Nontrivial V] : ¬(⊥ : SetGraph V).Preconnected :=
--   bot_preconnected_iff_subsingleton.not.mpr <| not_subsingleton_iff_nontrivial.mpr ‹_›

lemma top_preconnected : (⊤ : SetGraph V).Preconnected := @fun x y hx hy => by
  if h : x = y then rw [h]; exact ⟨.nil (by trivial)⟩; else exact Adj.reachable h

theorem Iso.preconnected_iff {G : SetGraph V} {H : SetGraph V'} (e : G ≃g H) :
...
theorem Preconnected.exists_isPath {G : SetGraph V} (h : G.Preconnected) {u v : V}
...
theorem reachable_is_equivalence : Equivalence (fun u v ↦ G.Reachable u v) :=
  Equivalence.mk (fun u ↦ ⟨.nil (by simp)⟩) (@Reachable.symm _ G.toSetGraph)
    (@Reachable.trans _ G.toSetGraph)
...
-- lemma bot_not_connected [Nontrivial V] : ¬(⊥ : SetGraph V).Connected := by
...
/-- The quotient of `V` by the `SetGraph.Reachable` relation gives the connected
...
theorem _root_.SetGraph.Preconnected.subsingleton_connectedComponent (h : G.Preconnected) :
...
-- lemma mem_coe_supp_of_adj {v w : V} {H : SetGraph V} {c : ConnectedComponent H.coe}
...
def IsBridge (G : SetGraph V) (e : Sym2 V) : Prop :=
  e ∈ G.edgeSet ∧
...
theorem reachable_delete_edges_iff_exists_walk {v w v' w' : V} :
  ...
  · rintro ⟨p⟩
    ...
    rintro ⟨e, h, rfl⟩
    have := p.edges_subset_edgeSet h
    simp only [edgeSet_sdiff, edgeSet_fromEdgeSet, Set.mem_diff, mem_edgeSet, Set.mem_singleton_iff,
