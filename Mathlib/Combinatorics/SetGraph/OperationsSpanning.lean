...
variable {V : Type*} (G : SpanningGraph V) (s t : V)

namespace SetGraph.Iso
...
def replaceVertex : SpanningGraph V :=
  SpanningGraph.mk' (fun v w ↦ if v = t then if w = t then False else G.Adj s w
                      else if w = t then G.Adj v s else G.Adj v w)
                  (fun v w ↦  by dsimp only; split_ifs <;> simp [adj_comm])

    (fun v h' ↦ by dsimp at h'; split_ifs at h' ; exact G.loopless _ h')

@[simp]
lemma replaceVertex_adj {v w : V} :
    (G.replaceVertex s t).Adj v w ↔ (if v = t then if w = t then False else G.Adj s w
                      else if w = t then G.Adj v s else G.Adj v w) := Iff.rfl

/-- There is never an `s-t` edge in `G.replaceVertex s t`. -/
lemma not_adj_replaceVertex_same : ¬(G.replaceVertex s t).Adj s t :=
...
theorem edgeSet_replaceVertex_of_not_adj (hn : ¬G.Adj s t)  :
  ...
  intros; dsimp; split_ifs; exacts [by simp_all, by aesop, by rw [adj_comm]; aesop, by aesop]
...
instance  : DecidableRel (G.replaceVertex s t).Adj := by
  intro u v
  rw [replaceVertex_adj]
  infer_instance
...
def edge : SpanningGraph V := fromEdgeSet {s(s, t)}
...
theorem edgeSet_sup_edge_le {H : SpanningGraph V} (hH : H ≤ G ⊔ edge s t)
...
variable [DecidableEq V] in
instance : Fintype (edge s t).edgeSet := by rw [edge, edgeSet_fromEdgeSet]; infer_instance

theorem edgeFinset_sup_edge [Fintype ((G ⊔ edge s t).edgeSet )] (hn : ¬G.Adj s t) (h : s ≠ t) :
    (G ⊔ edge s t).edgeFinset = G.edgeFinset.cons s(s, t) (by simp_all) := by
  letI := Classical.decEq V
  simp_rw [coe_sup, edgeFinset_sup, cons_eq_insert, insert_eq, union_comm]
  simp_rw [edgeFinset, edge_edgeSet_of_ne h]; rfl

theorem card_edgeFinset_sup_edge [Fintype ((G ⊔ edge s t).edgeSet )] (hn : ¬G.Adj s t) (h : s ≠ t) :
