...
- `SetGraph.sum_degrees_eq_twice_card_edges` is the degree-sum formula.
- `SetGraph.even_card_odd_degree_vertices` is the handshaking lemma.
- `SetGraph.odd_card_odd_degree_vertices_ne` is that the number of odd-degree
  vertices different from a given odd-degree vertex is odd.
- `SetGraph.exists_ne_odd_degree_of_exists_odd_degree` is that the existence of an
...
My graphs, sums, degree-sum formula, handshaking lemma
-/
...
more specifically refer to `SetGraph.even_card_odd_degree_vertices`. -/
...
See `SetGraph.sum_degrees_eq_twice_card_edges` for the general version. -/
...
/-- The handshaking lemma.  See also `SetGraph.sum_degrees_eq_twice_card_edges`. -/
