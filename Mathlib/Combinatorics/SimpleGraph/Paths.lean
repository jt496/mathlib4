...
lemma IsPath.mem_support_iff_exists_append {p : G.Walk u v} (hp : p.IsPath) :
    w ∈ p.support ↔ ∃ (q : G.Walk u w) (r : G.Walk w v), q.IsPath ∧ r.IsPath ∧ p = q ++ r := by
...
lemma IsPath.ne_of_mem_support_of_append {p : G.Walk u v} {q : G.Walk v w}
    (hpq : (p ++ q).IsPath) {x y : V} (hyv : y ≠ v) (hx : x ∈ p.support) (hy : y ∈ q.support) :
    x ≠ y := by
  rintro rfl
  have hq : ¬q.Nil := by
    intro hq
